...
#'
#' @description
#' Given an [`xkcd`] comic number, this function retrieves a JSON object describing
#' that comic from the xkcd website
#'
#'
#'
...
#'
#' Given an [`xkcd`] object, this [`base::plot`] method retrieve the image file associated with
#' this comic from the xkcd website and displays it in the currently active
#' graphics device.
#'
...
plot.xkcd <- function(x, ...) {
  ...
  if(img_type == "png") {
    ...
    stop("Unknown omage format", img_type)
  }
...
# In console, use devtools::load_all() to load code for checking, but don't run
# the code chunk as normal.
